...
tab1, tab2 = st.tabs(["Popular Books", "Book-Recommender"])
with tab1:
    
    """image = Image.open("img1.jpg")
    new_image = image.resize((600,250))
    st.image(new_image)"""
    ...
    image_url = popular_df['image_url_m'].tolist()
    book_title = popular_df['book_title'].tolist() 
    book_author = popular_df['book_author'].tolist()
    total_ratings = popular_df['book_rating'].tolist()
...
with tab2:
    ...
    new_image2 = image2.resize((600,250))
    st.image(new_image2)
    ...
    def fetch_poster(suggestion):
        ...
        for name in book_name: 
            ids = np.where(books['book_title'] == name)[0][0]
            ids_index.append(ids)

        for idx in ids_index:
            url = books.iloc[idx]['image_url_l']
